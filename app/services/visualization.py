...
def plot_data(x, y, labels, title, x_label, y_label, color_map=None, subplot_index=1, total_plots=1):
    ...
    for i, label in enumerate(np.unique(labels)):
        ...
        plt.plot(x[section_mask], y[section_mask], marker='o', color=color_map[i], linestyle='-',
...
def plot_pipeline_data(file_path):
    ...
    df['Cumulative Segment Index'] = np.arange(len(df))
...
def plot_all_columns(file_path):
    ...
    # Создание уникального кумулятивного индекса для каждого сегмента
    df['Cumulative Segment Index'] = np.arange(len(df))
    ...
    for i, column in enumerate(numeric_columns, 1):
        plot_data(df['Cumulative Segment Index'], df[column], df['Section Index'],
                  column, 'Cumulative Segment Index', 'Value',
                  color_map=colors, subplot_index=i, total_plots=total_plots)
